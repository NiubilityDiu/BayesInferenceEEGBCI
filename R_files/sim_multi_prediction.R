# Produce prediction accuracy comparison among Bayes and other ML methods 
...
bayes_sseq = bayes_freq_table_design_num(
  ...
  scenario_name, zeta_bayes, num_letter, 'simulation_multiple'
)
...
cnn = exist_ml_freq_table_design_num(
  parent_path, 'EEGCNN', design_num, dataset_num, data_type,
  num_rep_fit, num_rep_pred, scenario_name, 0, num_letter
)
...
output_file_dir = paste(parent_path, '/EEG_MATLAB_data/SIM_summary/sim_',
                        ...
                        num_rep_pred,  '_summary_', scenario_name, '_zeta_', zeta_bayes, sep="")
...
sim_summary_multi = readRDS(paste(output_file_dir, '.RDS', sep=""))
