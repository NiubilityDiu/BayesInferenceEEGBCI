...
class ReArrangeBetaSigma:
    ...
    # which applies tensorflow and tensorflow-probability.
    def create_design_matrix_bayes(self, letter_dim, repetition_dim, channel_dim=1):
        ...
        dm = np.tile(dm[np.newaxis, np.newaxis, :, :], [letter_dim, channel_dim, 1, 1])
        # dm should have output shape (19, 16, 920, 4500)
        return dm

    def create_joint_beta_tilta(self, letter_dim, repet_dim, beta_combined, id_beta, design_x, channel_dim=1):

        r"""
        letter_dim: integer
        repet_dim: integer
        beta_combined: array_like
            should have the input shape (letter_dim, channel_dim, self.rep_dim * self.num_rep * self.n_length, 1)
        id_beta: array_like or None
            permute id by the eeg_code.
            If none, then it implies that the beta has already been permuted by id_beta.
        design_x: array_like
            sparse matrix with input shape (letter_dim, channel_dim, seq_length, self.rep_dim * self.num_rep * self.n_length)
        """

        if id_beta is not None:
            beta_permuted = self.permute_beta_by_type(letter_dim, repet_dim, beta_combined, id_beta, channel_dim)
        else:
            beta_permuted = np.copy(beta_combined)

        return np.matmul(design_x, beta_permuted)
...
class WLSOpt(EEGPreFun):
    ...
    def import_eeg_processed_dat_wls(self, file_subscript,
        ...
        [eeg_signals, eeg_code, eeg_type] = \
            self.import_eeg_processed_dat(file_subscript, reshape_to_1d=False)
    ...
    def partial_log_prob_eeg_conol_test_fn(
        ...
        design_x = self.rearrange.create_design_matrix_bayes(letter_dim_test, repet_dim_test)
